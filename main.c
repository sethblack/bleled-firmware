...
#define MIN_CONN_INTERVAL               MSEC_TO_UNITS(60, UNIT_1_25_MS)            /**< Minimum acceptable connection interval (0.5 seconds). */
...
static void advertising_init(void) {
    ...
    ble_uuid_t          adv_uuids[] = {{CANDLE_UUID_SERVICE, m_candle.uuid_type}};
...
static void services_init(void) {
    uint32_t err_code;
    ble_candle_init_t init;
    
    init.candle_write_handler = candle_write_handler;
    
    err_code = ble_candle_service_init(&m_candle, &init);
    APP_ERROR_CHECK(err_code);
}
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt) {
    ...
    ble_candle_on_ble_evt(&m_candle, p_ble_evt);
}
